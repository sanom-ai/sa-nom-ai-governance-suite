...
Current runtime support now materializes the first governed trigger actions directly in execution metadata: terminal outcomes plus `require_approval(...)`, `apply_policy_pack(...)`, `rewrite_tone(...)`, and `log_evidence(...)`.
