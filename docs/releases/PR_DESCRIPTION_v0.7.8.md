## Summary
This PR delivers 0.7.8 as the private-first tablet command-surface release.
...
- Governance popup and Control Room launcher polish for clearer advanced-tool discovery on tablet and PC
- role-based lane emphasis so each tablet persona sees the most important next lane first
## Why This Change
The goal of 0.7.8 is to make SA-NOM feel credible on a private organizational tablet, not only on a desktop development screen.
Before this change, the command surface had already become simpler and more product-like in 0.7.7.
...
- advanced governance tools needed to stay discoverable without cluttering Home
...
- [x] Current full-suite status: 363 passed
- [x] Current coverage posture: 85.04%
- [x] Docs updated when behavior changed
## Notes
- this PR is the private-first tablet productization milestone for 0.7.8
- the command surface remains simple while session continuity, touch flow, and persona routing become clearer on tablet
- advanced governance tooling now has a more discoverable popup launcher without falling back to a cramped dropdown
