...
This release turns the new command surface into a touch-first operating shell for organizational tablet use while preserving the founder doctrine:
...
- kept the Home surface clean while preserving the current dark + beige/gold command aesthetic
...
### Session Recovery And Persona Emphasis Hardening

- added stronger session continuity states such as `ready`, `renew soon`, `idle lock soon`, and `reconnect now`
- added role-aware lane emphasis metadata so tablet users can see which lane to open first, keep close, or treat as supporting context
- added manual `renew session` behavior from the command surface
- added safe GET-based session recovery so dashboard refresh paths can reconnect automatically when the signed session cools down but a private access token is still available

## Why This Release Matters

`v0.7.8` matters because the founder vision is not just about having a dashboard that looks clean on a laptop.
...
- the command surface stays simple while deeper governance tooling remains protected in Control Room
