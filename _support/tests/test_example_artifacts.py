...
def test_production_line_exception_scenario_example_preserves_human_production_boundary():
    payload = json.loads((EXAMPLES_DIR / "production_line_exception_scenario.example.json").read_text(encoding="utf-8"))

    assert payload["role_pack"] == "production_line_exception_pack"
    assert payload["reporting_line"] == "PRODUCTION"
    assert "approve_recovery_exception" in payload["wait_human_actions"]
    assert "approve_schedule_override" in payload["wait_human_actions"]
    assert "override_quality_hold" in payload["forbidden_actions"]
...
def test_quality_audit_readiness_scenario_example_preserves_human_quality_boundary():
    payload = json.loads((EXAMPLES_DIR / "quality_audit_readiness_scenario.example.json").read_text(encoding="utf-8"))

    assert payload["role_pack"] == "quality_audit_readiness_pack"
    assert payload["reporting_line"] == "QUALITY"
    assert "approve_release_exception" in payload["wait_human_actions"]
    assert "approve_deviation_waiver" in payload["wait_human_actions"]
    assert "release_quality_hold" in payload["forbidden_actions"]
...
def test_delivery_readiness_scenario_example_preserves_human_delivery_boundary():
    ...
    assert payload["role_pack"] == "delivery_readiness_pack"
    assert payload["reporting_line"] == "DELIVERY"
    assert "approve_dispatch_exception" in payload["wait_human_actions"]
    assert "approve_customer_commitment_change" in payload["wait_human_actions"]
    assert "release_shipment" in payload["forbidden_actions"]
...
def test_external_audit_response_scenario_example_preserves_human_audit_boundary():
    payload = json.loads((EXAMPLES_DIR / "external_audit_response_scenario.example.json").read_text(encoding="utf-8"))

    assert payload["role_pack"] == "external_audit_response_pack"
    assert payload["reporting_line"] == "AUDIT"
    assert "approve_audit_response_exception" in payload["wait_human_actions"]
    assert "approve_regulatory_response" in payload["wait_human_actions"]
    assert "close_audit_finding" in payload["forbidden_actions"]
...
def test_regulator_response_scenario_example_preserves_human_regulator_boundary():
    ...
    assert payload["role_pack"] == "regulator_response_pack"
    assert payload["reporting_line"] == "COMPLIANCE"
    assert "approve_regulator_response_exception" in payload["wait_human_actions"]
    assert "approve_regulatory_filing" in payload["wait_human_actions"]
    assert "release_official_regulator_response" in payload["forbidden_actions"]
...
def test_private_rule_position_example_describes_core_studio_capability():
    ...
    assert payload["studio_entrypoint"] == "Role Private Studio"
    assert payload["role_type"] == "organization_defined_governed_hat"
